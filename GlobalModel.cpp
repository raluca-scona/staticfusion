...
void GlobalModel::initialise(const FeedbackBuffer & rawFeedback,
                             ...
                             GPUTexture * weightedImage,
                             Eigen::Matrix4f currPose)
{
    initProgram->Bind();

    initProgram->setUniform(Uniform("pose", currPose));
                                   ...
                                   const int timeDelta)
{
    ...
    for(int r=0; r<4; ++r ) {
        for(int c=0; c<4; ++c ) {
            mat(r,c) = (float) mvp.m[c*4+r];
        }
    }

    program->setUniform(Uniform("MVP", mat));

    program->setUniform(Uniform("threshold", threshold));

    program->setUniform(Uniform("colorType", (drawNormals ? 1 : drawColors ? 2 : drawTimes ? 3 : drawConf ? 4 : 0)));
                       ...
                       const float weighting)
{
    ...
    dataProgram->setUniform(Uniform("probIsStaticSamp", 7));
    ...
    glBindTexture(GL_TEXTURE_2D, rgb->texture->tid);   //cSampler

    glActiveTexture(GL_TEXTURE1);
    glBindTexture(GL_TEXTURE_2D, depthRaw->texture->tid); //drSampler

    glActiveTexture(GL_TEXTURE2);
    glBindTexture(GL_TEXTURE_2D, depthFiltered->texture->tid); //drfSampler

    glActiveTexture(GL_TEXTURE3);
    glBindTexture(GL_TEXTURE_2D, indexMap->texture->tid);  //indexSampler

    glActiveTexture(GL_TEXTURE4);
    glBindTexture(GL_TEXTURE_2D, vertConfMap->texture->tid); //vertConfSampler

    glActiveTexture(GL_TEXTURE5);
    glBindTexture(GL_TEXTURE_2D, colorTimeMap->texture->tid); //colorTimeSampler

    glActiveTexture(GL_TEXTURE6);
    glBindTexture(GL_TEXTURE_2D, normRadMap->texture->tid); //normRadSampler

    glActiveTexture(GL_TEXTURE7);
    glBindTexture(GL_TEXTURE_2D, weightedImage->texture->tid);  //probIsStaticSamp
    ...
    updateProgram->setUniform(Uniform("stableThreshold", confThreshold));
