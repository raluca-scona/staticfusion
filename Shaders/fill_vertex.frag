...
void main()
{
    ...
    float probIsStatic = float(textureLod(probIsStaticSamp, texcoord, 0.0));
    ...
    if(sample.z == 0 || passthrough == 1)
    {
        ...
        if (probIsStatic > 0.6) {
            FragColor = vec4(getVertex(int(texcoord.x * cols), int(texcoord.y * rows), z), 0.12); 
        } else {
            FragColor = vec4(getVertex(int(texcoord.x * cols), int(texcoord.y * rows), 0.0), 0.12); 
        }
