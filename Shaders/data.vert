...
uniform sampler2D probIsStaticSamp;
...
void main()
{
    ...
    float x = texcoord.x * cols; //rescale from [0, 1] to [640, 480]
    float y = texcoord.y * rows;

    //Vertex position integrated into model transformed to global coords
    vec3 vPosLocal = getVertex(texcoord.xy, x, y, cam, drSampler);   //current point, not model. drsampler meaning raw depth is used
    vPosition = pose * vec4(vPosLocal, 1);   //point from current raw 3d depth stream expressed in world coords
    
    //Filtered position ONLY used for normal and radius calculation
    vec3 vPosition_f = getVertex(texcoord.xy, x, y, cam, drfSampler);

    float probIsStatic = float(textureLod(probIsStaticSamp, texcoord.xy, 0.0));
    ...
    //Confidence
    float radialConf = confidence(x, y, weighting);
    vPosition.w = min(probIsStatic, min(weighting, radialConf));  //depends on radial distance of point from center of camera and norm of velocity vector
    vColor.y = 1.0;   //store history here
       ...
       vPosLocal.z <= maxDepth)
    {
	    ...
	    float indexXStep = (1.0f / (cols * scale)) * 0.5f;  //scale usually =1 -> this is 1/(2cols)
	    ...
	    //searching for which point in the index map to fuse with
	    for(float i = texcoord.x - (scale * indexXStep * windowMultiplier); i < texcoord.x + (scale * indexXStep * windowMultiplier); i += indexXStep)
	    {
	        ...
	        {
	           uint current = uint(textureLod(indexSampler, vec2(i, j), 0.0));   //there is a matching point in the indexSampler
	    ...
	    else 
	    {
         ...
         //basically only adding new points with low confidence if their probIsStatic is high enough
          if (probIsStatic > 0.5)
            vPosition.w = 0.08;
	    }
    }
