...
{
    const float maxRadDist = 200; //sqrt((width * 0.5)^2 + (height * 0.5)^2)
    const float twoSigmaSquared = 2*0.72; //2*(0.6^2) from paper
    
    vec2 pixelPosCentered = vec2(x, y) - cam.xy;
   
    float radialDist = sqrt(dot(pixelPosCentered, pixelPosCentered)) / maxRadDist;
    
    return ( exp((-(radialDist * radialDist) / twoSigmaSquared)) ); 
